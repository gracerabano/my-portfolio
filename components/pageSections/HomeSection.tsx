...
export default function HomeSection(props: homeProps) {
  return(
    <div id="home" className={`flex flex-col mt-10 justify-start w-full backdrop-blur-md bg-sheer rounded-xl px-12 py-12 transform transition-all duration-1000 delay-500 ${props.isLoaded ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
      {props.isLoaded && (
        <>
          <h1 className="inline-block text-left px-2 text-4xl font-bold mr-12">Hello! Nice to Meet You!</h1>
          <p className="block mt-4 mr-12 px-2 text-gray-700 leading-6">
            ...
            Though my focus is web dev, I'm also interested in branching out into other fields, such as IT and cybersecurity.
          </p>
          <p className="block mt-4 mr-12 px-2 text-gray-700 leading-6">
